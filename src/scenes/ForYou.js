...
class ForYou extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      recentlyWatched: [
        ...
        ...recentlyWatchedSeasonsSelector(oneMonthAgo)({ seasons: this.props.seasons })
      ].sort((a, b) => (a.lastWatched > b.lastWatched) ? -1 : 1)
    }
  }
  ...
  render() {
    if (this.state.inProgress.length > 0 || this.state.recentlyWatched.length > 0) {
      return (
    ...
    } else if (Object.values(PROVIDERS).filter(provider => this.props.auth[provider].token !== null).length === 0) {
      return (
        <div className='ForYou'>
          <h2>Get started</h2>
    ...
    } else if (Object.entries(this.props.movies).length === 0 && Object.entries(this.props.shows).length === 0) {
      return (
        <div className='ForYou'>
          <h2>Get started</h2>
